...
module.exports = {
	tasksAllowed : ['init', 'i', 'generate', 'g', 'build', 'b', 'watch', 'w', 'server' , 's', 'developer', 'd', 'h', 'help', 'install', 'inst'],
	...
	install: {
		description: 'Install dependencies from bower and svn',
		params: [
			{name: '-b', description: 'Only install from bower'},
			{name: '-s', description: 'Only install from subversion' }],
		run: function(paramData) {
			install(paramData);
		},
		processParams: function(args) {
			return {
				onlyBower: this.that.checkParamExistence(args, '-b'),
				onlySubversion: this.that.checkParamExistence(args, '-s')
			}
		}
	},
	inst: {
